...
class DemandForecastingAgent(BaseAgent):
    ...
    def prepare_features(self, sku: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(7, len(data)):  # Use 7-day lookback
            ...
            day_of_week = current_date.weekday()  # Fixed: weekday() method instead of dayofweek
    ...
    def generate_forecast(self, sku: str, horizon: int = 14) -> Optional[DemandForecast]:
        ...
        for day in range(horizon):
            ...
            day_of_week = future_date.weekday()  # Fixed: weekday() method
...
class StockLevelMonitoringAgent(BaseAgent):
    ...
    def update_inventory_data(self, data: Dict[str, Any]):
        ...
        location = data.get('location', 'main')  # Default location
    ...
    def check_stock_levels(self):
        ...
        for key, inventory in self.inventory_data.items():
            try:
                sku = inventory.sku
                location = getattr(inventory, 'location', 'main')  # Safe access to location
...
class SeasonalAdjustmentAgent(BaseAgent):
    ...
    def analyze_seasonal_patterns(self, demand_data: Dict[str, Any]):
        ...
        day_of_week = current_date.weekday()  # Fixed: weekday() method
...
class InventoryOptimizationSystem:
    ...
    def start_system(self):
        ...
        print("Inventory initialization completed!")
        ...
        for sku in self.products.keys():
            try:
                ...
                stock_agent.update_inventory_data({
                    'sku': sku,
                    'location': 'main',  # Ensure location is always provided
                    'current_stock': initial_stock,
                    'reserved_stock': reserved_stock
                })
                
                print(f"Initialized inventory for {sku}: {initial_stock} units at location 'main'")
    ...
    def verify_system_integrity(self):
        ...
        print(f"✓ Running agents: {running_agents}/{len(self.agents)}")
        
        # Check inventory data
        stock_agent = self.agents["stock_monitoring_agent"]
        if hasattr(stock_agent, 'inventory_data'):
            print(f"✓ Inventory items tracked: {len(stock_agent.inventory_data)}")
            
            # Verify inventory data integrity
            for key, inventory in stock_agent.inventory_data.items():
                if not hasattr(inventory, 'location') or inventory.location is None:
                    print(f"⚠ Warning: Missing location for {key}")
                else:
                    print(f"✓ {inventory.sku} at {inventory.location}: {inventory.current_stock} units")
        
        # Check forecasting agent
        demand_agent = self.agents["demand_forecasting_agent"]
        if hasattr(demand_agent, 'historical_data'):
            print(f"✓ Historical data for {len(demand_agent.historical_data)} products")
    ...
    def run_simulation(self, duration_minutes: int = 1):
        ...
        while time.time() < end_time and self.running:
            try:
                # Print periodic status updates (less frequent to reduce noise)
                if update_count % 3 == 0:  # Every 30 seconds instead of 10
                    print(f"\n--- Update {update_count // 3 + 1} ---")
                    status = self.get_system_status()
                    metrics = self.get_performance_metrics()
                    print(f"Agents running: {status.get('agents', {})}")
    ...
    try:
        ...
        try:
            ...
            if hasattr(demand_agent, 'forecast_cache') and demand_agent.forecast_cache:
                ...
                for sku, forecast in demand_agent.forecast_cache.items():
                    avg_demand = np.mean(forecast.predicted_demand[:7])
                    print(f"  {sku}: Next 7 days avg demand = {avg_demand:.1f}")
            else:
                print("\n📊 No forecasts generated yet (system may need more time)")
        except Exception as e:
        ...
        try:
            ...
            if hasattr(reorder_agent, 'reorder_points') and reorder_agent.reorder_points:
                ...
                print("\n🎯 No reorder points calculated yet")
        except Exception as e:
            print(f"Error displaying reorder points: {e}")
        ...
        try:
            ...
            if hasattr(safety_stock_agent, 'safety_stocks') and safety_stock_agent.safety_stocks:
                print(f"\n🛡️ Safety stocks optimized:")
                for sku, ss in safety_stock_agent.safety_stocks.items():
                    print(f"  {sku}: Safety Stock = {ss:.1f}")
            else:
                print("\n🛡️ No safety stocks calculated yet")
        except Exception as e:
            print(f"Error displaying safety stocks: {e}")
